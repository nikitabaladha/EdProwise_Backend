...
async function sendSchoolRequestQuoteEmail(
  ...
  try {
    ...
    const viewQuoteUrl = `${frontendUrl.replace(
      /\/+$/,
      ""
    )}/school-dashboard/procurement-services/view-order-history?orderNumber=${encodeURIComponent(orderNumber)}`;
...
async function sendEmailsToSellers(
  ...
  try {
    ...
    const sellerDashboardUrl = `${frontendUrl.replace(
      /\/+$/,
      ""
    )}/seller-dashboard/procurement-services/view-order-history?orderNumber=${encodeURIComponent(orderNumber)}`;
    const contactUrl = `${frontendUrl.replace(/\/+$/, "")}/contact-us`;
...
async function create(req, res) {
  ...
  try {
    ...
    for (const product of products) {
      ...
      if (!sellerOrderNumbers.has(cartEntry.sellerId.toString())) {
        const newOrderNumber = await generateOrderNumber(); // Await here
        sellerOrderNumbers.set(
          cartEntry.sellerId.toString(),
          newOrderNumber // Now this is the resolved string, not a Promise
        );
      }
