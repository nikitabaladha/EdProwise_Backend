...
// async function sendSchoolRequestQuoteEmail(schoolName, schoolEmail, orderDetails) {
//   let hasError = false;
//   let message = "";
...
//     // 2. Email template
//     const emailTemplate = await SchoolOrderPlaceEmailTemplate.findOne();
//     if (!emailTemplate) {
...
//     const sendUrl = `${frontendUrl.replace(/\/+$/, '')}/school-dashboard/procurement-services/track-order-history`;

//     // 7. Build final email body
//     const emailContent = emailTemplate.content
//       .replace(/{orderNumber}/g, orderNumber)
//       .replace(/{schoolName}/g, schoolName)
//       .replace(/{orderDetails}/g, quoteDetailsHtml)
//       .replace(/{mailForm}/g, smtpSettings.mailFromName)
//       .replace(/{app_url}/g,sendUrl );
...
// async function sendEmailsToSellers(sellerName, sellerEmail, orderDetails) {
//   try {
//     console.log("Starting seller email process...");
...
//     if (!emailTemplate) {
//       console.error("Seller email template not found");
//       return { hasError: true, message: "Seller email template not found" };
//     }
...
//     const { orderNumber, products, deliveryAddress, deliveryLocation, 
//             deliveryLandMark, deliveryPincode, expectedDeliveryDate } = orderDetails;

//     const productHtml = products.map((product, index) => `
//       <tr>
//         <td>${index + 1}</td>
//         <td>${product.subcategoryName}</td>
//         <td>${product.hsnSacc}</td>
//         <td>${product.quantity}</td>
//         <td>₹${product.finalRate}</td>
...
//     const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
//     const sellerDashboardUrl = `${frontendUrl.replace(/\/+$/, '')}/seller-dashboard/procurement-services/track-order-history`;

//     const emailContent = emailTemplate.content
//       .replace(/{sellerCompanyName}/g, sellerName)
//       .replace(/{orderDetails}/g, orderDetailsTable)
//       .replace(/{mailForm}/g, smtpSettings.mailFromName)
//       .replace(/{app_url}/g, sellerDashboardUrl);

//     await transporter.sendMail({
//       from: `"${smtpSettings.mailFromName}" <${smtpSettings.mailFromAddress}>`,
//       to: sellerEmail,
//       subject: emailTemplate.subject.replace('{orderNumber}', orderNumber),
//       html: emailContent,
//     });
...
async function sendSchoolRequestQuoteEmail(schoolName, schoolEmail, orderDetails) {
  ...
  try {
    ...
    const { orderNumber, products } = orderDetails;
    const enquiryNumber = products.length > 0 ? products[0].enquiryNumber : 'N/A';
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    const viewQuoteUrl = `${frontendUrl.replace(/\/+$/, '')}/school-dashboard/procurement-services/track-order-history`;
...
async function sendEmailsToSellers(sellerName, sellerEmail, schoolName, orderDetails) {
  try {
...
//       return res.status(401).json({
//         hasError: true,
//         message: "Access denied: You do not have permission to request a quote.",
//       });
//     }
