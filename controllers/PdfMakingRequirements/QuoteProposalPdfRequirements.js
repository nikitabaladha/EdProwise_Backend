...
// //on the basis of sellerID  form seller Profile table i want companyName,address, landmark, cityStateCountry, gstin,pan,contactNo,emailId
...
//     // Fetch Quote Proposal details (Contains sellerId)
//     const quoteProposal = await QuoteProposal.findOne({
//       enquiryNumber: quoteRequest.enquiryNumber,
...
//     // Fetch Seller Profile details using sellerId from QuoteProposal
...
//     // Fetch the only EdprowiseProfile in the database (since there is only one)
...
//           sellerProfile?.address ? `, ${sellerProfile.landmark}` : ""
//         }`.trim(),
...
async function QuoteProposalPdfRequirements(req, res) {
  try {
    ...
    // Fetch Order Details from Seller using schoolId, sellerId, and orderNumber
