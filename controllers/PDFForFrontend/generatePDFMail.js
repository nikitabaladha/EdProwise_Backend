...
async function generatePDFMail(htmlFilePath, dynamicData = {}, outputPath) {
  ...
  try {
    ...
    // Logo handling
    const logoPath = path.join(__dirname, "../../Images/edprowiseLogoImages/EdProwiseNewLogo.png");
    if (!fs.existsSync(logoPath)) {
      throw new Error(`Logo not found at ${logoPath}`);
    }

    if (!logoImageCache) {
      logoImageCache = `data:image/png;base64,${fs.readFileSync(logoPath, "base64")}`;
    }
    ...
    // Render HTML
    const htmlContent = ejs.render(fs.readFileSync(htmlFilePath, "utf8"), dynamicData);
