...
const updateSchoolFeesStatus = async (req, res) => {
  ...
  try {
    const fees = await SchoolFees.findOne({ _id: id, schoolId }).session(session);
    if (!fees) {
      await session.abortTransaction();
      session.endSession();
      return res.status(404).json({
        hasError: true,
        message: 'School fees record not found or you do not have permission to update this record.',
      });
    }
