...
const updateConcessionStatus = async (req, res) => {
  ...
  try {
    const concession = await ConcessionForm.findOne({ _id: id, schoolId }).session(session);
    if (!concession) {
      await session.abortTransaction();
      session.endSession();
      return res.status(404).json({
        hasError: true,
        message: 'Concession form not found or you do not have permission to update this record.',
      });
    }
