  ...
  try {
    ...
    const result = await StudentRegistration.aggregate([
      ...
      },
      {
        $project: {
          ...
          easebuzzTxnId:'$RegistrationPayment.easebuzzTxnId',
          easebuzzId: '$RegistrationPayment.easebuzzId',
          hash:'$RegistrationPayment.hash',
    ...
    if (!result || result.length === 0) {
      return res.status(404).json({
        hasError: true,
        message: 'No student or payment data found for the provided Student ID and Receipt Number.',
      });
    }
