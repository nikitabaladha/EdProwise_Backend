...
//       .select('AdmissionNumber firstName lastName academicHistory TCStatus TCStatusDate') 
//       .lean();
//     if (!students.length) {
...
//       const paidInstallmentNames = Object.keys(paymentsByInstallment);
//       const unpaidInstallments = expectedInstallments.filter(
//         (instName) => !paidInstallmentNames.includes(instName)
//       );

//       for (const instName of unpaidInstallments) {
//         const structureInst = feesStructures
//           .flatMap((structure) => structure.installments)
//           .find((inst) => inst.name === instName);
...
//         const installmentBalance = initialFeesDue - installmentConcession;
...
export const getAllStudentFeesDue = async (req, res) => {
  try {
    ...
    // Fetch students with optional class and section filtering
