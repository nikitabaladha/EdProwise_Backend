...
//         hasError: true,
//         message: "Invalid academic year format. Please use either YYYY or YYYY-YYYY format.",
//       });
//     }
...
//       return res.status(400).json({
//         hasError: true,
//         message: "Invalid academic year sequence. The second year should be exactly +1 of the first year.",
//       });
//     }
...
//     const copiedShifts = await copyPreviousShifts(schoolId, academicYear, prevAcademicYear, session);
//     const copiedAdmissionPrefixes = await copyPreviousAdmissionPrefixes(schoolId, academicYear, prevAcademicYear, session);
//     const copiedRegistrationPrefixes = await copyPreviousRegistrationPrefixes(schoolId, academicYear, prevAcademicYear, session);
//     const copiedClasses = await copyPreviousClassesAndSections(schoolId, academicYear, prevAcademicYear, session);
//     const copiedFeesTypes = await copyPreviousFeesTypes(schoolId, academicYear, prevAcademicYear, session);
//     const copiedFines = await copyPreviousFines(schoolId, academicYear, prevAcademicYear, session);
//     const copiedOneTimeFees = await copyPreviousOneTimeFees(schoolId, academicYear, prevAcademicYear, session);
//     const copiedFeesStructures = await copyPreviousFeesStructure(schoolId, academicYear, prevAcademicYear, session);
//     const copiedBoardExamFees = await copyPreviousBoardExamFees(schoolId, academicYear, prevAcademicYear, session);
//     const copiedBoardRegistrationFees = await copyPreviousBoardRegistrationFees(schoolId, academicYear, prevAcademicYear, session);
...
export const createFeesManagementYear = async (req, res) => {
  ...
  try {
    ...
    const currentAcademicYear = currentMonth >= 3 
    ...
    if (!existingCurrentYear) {
      ...
      const copiedShifts = await copyPreviousShifts(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedAdmissionPrefixes = await copyPreviousAdmissionPrefixes(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedRegistrationPrefixes = await copyPreviousRegistrationPrefixes(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedClasses = await copyPreviousClassesAndSections(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedFeesTypes = await copyPreviousFeesTypes(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedFines = await copyPreviousFines(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedOneTimeFees = await copyPreviousOneTimeFees(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedFeesStructures = await copyPreviousFeesStructure(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedBoardExamFees = await copyPreviousBoardExamFees(schoolId, currentAcademicYear, prevAcademicYear, session);
      const copiedBoardRegistrationFees = await copyPreviousBoardRegistrationFees(schoolId, currentAcademicYear, prevAcademicYear, session);
    ...
    const finalStartDate = startDate ? new Date(startDate) : new Date(startYear, 3, 1); // April 1
    const finalEndDate = endDate ? new Date(endDate) : new Date(endYear, 2, 31); // March 31

    const { error } = FeesManagementYearValidator.validate({ schoolId, academicYear, startDate: finalStartDate, endDate: finalEndDate });
    if (error?.details?.length) {
      await session.abortTransaction();
      session.endSession();
      const errorMessages = error.details.map((err) => err.message).join(", ");
