...
async function updateStatus(req, res) {
  try {
    ...
    if (supplierStatus === "Delivered") {
      ...
      await sendSchooldeliverdEmail({schoolDetails,productDetails,deliveryDetails});
      await sendEmailsToSellers({sellerDetails,productDetails,deliveryDetails});
    }
...
async function sendSchooldeliverdEmail({ schoolDetails, productDetails, deliveryDetails }) {
  let hasError = false;
  let message = "";
  ...
  try {
    ...
    const pdfPath = path.join(__dirname, '../../Documents/SchoolPanFile/classsimple.pdf');
    ...
    const deliveryDetailsHtml = `
      <table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 100%;">
        <tr><th>Address</th>  <td>${deliveryDetails.deliveryAddress  || '-'}</td>  </tr>
        <tr><th>Location</th> <td>${deliveryDetails.deliveryLocation || '-'}</td> </tr>
        <tr><th>Landmark</th> <td>${deliveryDetails.deliveryLandMark || '-'}</td> </tr>
        <tr><th>Pincode</th>  <td>${deliveryDetails.deliveryPincode  || '-'}</td>  </tr>
        <tr><th>Expected Delivery Date</th><td>${deliveryDetails.expectedDeliveryDate || '-'}</td></tr>        
      </table>
    `;
              ...
              <!DOCTYPE html>
                ...
                <div class="outer-div">
                    <div class="email-container">
                        ...
                        <!-- Main Content -->
                        <div class="content">
                            <p class="message">Dear ${schoolDetails.schoolName},</p>
                            ...
                            <h3 class="heading">Enquiry Number: ${deliveryDetails.enquiryNumber}</h3>
...
async function sendEmailsToSellers({ sellerDetails, productDetails, deliveryDetails }) {
  try {
    const smtpSettings = await SMTPEmailSetting.findOne();
    if (!smtpSettings) {
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    const viewQuoteUrl = `${frontendUrl.replace(/\/+$/, '')}/seller-dashboard/procurement-services/track-order-history`;
    const contactUrl = `${frontendUrl.replace(/\/+$/, '')}/contact-us`;
  ...
  `;
          ...
          html: `
            ...
            <div class="outer-div">
                <div class="email-container">
                    ...
                    <!-- Main Content -->
                    <div class="content">
                        <p class="message">Dear ${sellerDetails.companyName},</p>
                        ...
                        <h3>Enquiry Number: ${deliveryDetails.enquiryNumber}</h3>
        ...
        console.log(`Order delivered email sent successfully to seller ${sellerDetails.name}`);
