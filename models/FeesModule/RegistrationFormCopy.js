...
// studentRegistrationSchema.pre('findOneAndUpdate', async function (next) {
...
const studentRegistrationSchema = new Schema(
  {
    ...
    additionalComment: { type: String },
    reportStatus: [
      { type: String, enum: ["Paid", "Cancelled", "Cheque Return"] },
    ],
  },
  { timestamps: true }
);
  ...
  while (attempts > 0) {
    try {
      ...
      if (this.registrationNumber) {
        ...
        if (setting.type === "numeric" && setting.value != null) {
          importedSeq =
            parseInt(this.registrationNumber) - parseInt(setting.value);
        } else if (setting.type === "alphanumeric" && setting.prefix) {
          const prefix = setting.prefix;
          if (!this.registrationNumber.startsWith(prefix)) {
            throw new Error(
              `RegistrationNumber ${this.registrationNumber} does not match prefix ${prefix}`
            );
          }
          const numericPart = this.registrationNumber.replace(prefix, "");
          importedSeq = parseInt(numericPart) - parseInt(setting.number);
        ...
        const updatedCounter = await RegistrationCounter.findOneAndUpdate(
          { schoolId: this.schoolId },
          {
            $inc: {
              registrationSeq: 1,
              receiptSeq: this.paymentMode === "null" ? 0 : 1,
            },
          },
          { new: true, upsert: true }
        );
        ...
        if (setting.type === "numeric" && setting.value != null) {
          ...
          const baseNumber = parseInt(setting.number);
          this.registrationNumber = `${setting.prefix}${
            baseNumber + updatedCounter.registrationSeq
          }`;
        } else {
          throw new Error("Incomplete prefix setting.");
        }
      ...
      if (
        err.code === 11000 &&
        (err.message.includes("registrationNumber") ||
...
export default mongoose.model("StudentRegistrationCopy", studentRegistrationSchema);
